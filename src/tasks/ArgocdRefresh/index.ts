...
import { ArgoCDServiceProvider, ArgoCDApplication } from '../../common/argocd-service-provider';
...
async function run() {
    try {
        ...
        if (appAfter.conditions && appAfter.conditions.length > 0) {
            ...
            appAfter.conditions.forEach((condition: { type?: string; status?: string; severity?: string; message?: string; reason?: string; lastTransitionTime?: string }, index: number) => {
                ...
                if (condition.reason) console.log(`        Reason: ${condition.reason}`);
            });
        } else {
        ...
        const analysisResult = await comprehensiveStatusAnalysis(
            argoCDService, 
            applicationName, 
            project, 
            appBefore, 
...
interface StatusAnalysisOptions {
...
async function comprehensiveStatusAnalysis(
    argoCDService: ArgoCDServiceProvider,
    applicationName: string,
    project: string | undefined,
    appBefore: ArgoCDApplication,
    appAfter: ArgoCDApplication,
    options: StatusAnalysisOptions
): Promise<StatusAnalysisResult> {
...
async function handleUnknownSyncStatus(
    argoCDService: ArgoCDServiceProvider,
    applicationName: string,
    project: string | undefined,
    app: ArgoCDApplication,
    options: StatusAnalysisOptions
): Promise<StatusAnalysisResult & { app?: ArgoCDApplication }> {
    ...
    conditions.forEach((c: ApplicationCondition, index: number) => {
    ...
    const errorConditions = conditions.filter((c: ApplicationCondition) => c.severity === 'Error');
    const warningConditions = conditions.filter((c: ApplicationCondition) => c.severity === 'Warning');
    
    if (errorConditions.length > 0) {
        console.log('❌ Found error conditions:');
        errorConditions.forEach((c: ApplicationCondition) => {
    ...
    if (warningConditions.length > 0) {
        ...
        warningConditions.forEach((c: ApplicationCondition) => {
            console.log(`   - ${c.type}: ${c.message}`);
        });
...
async function comprehensiveSyncMonitoring(
    argoCDService: ArgoCDServiceProvider,
    applicationName: string,
    ...
    while (Date.now() - startTime < timeoutMs) {
        try {
            ...
            if (app.syncStatus === 'Failed' || app.syncStatus === 'Error') {
                ...
                const errorConditions = (app.conditions || []).filter((c: ApplicationCondition) => c.severity === 'Error');
...
async function validateApplicationHealthAndConditions(
    ...
    conditions.forEach((c, index: number) => {
        console.log(`   ${index + 1}. Type: ${c.type}, Status: ${c.status}, Severity: ${c.severity || 'None'}`);
        console.log(`      Message: ${c.message}`);
        if (c.reason) console.log(`      Reason: ${c.reason}`);
    });
    ...
    const errorConditions = conditions.filter(c => 
        ...
        c.type === 'SyncError' ||
        (c.message?.toLowerCase().includes('error')) ||
...
async function validateFinalApplicationState(
    app: ArgoCDApplication, 
    options: { failOnWarnings: boolean; failOnDegraded: boolean; failOnUnknown: boolean }
): Promise<StatusAnalysisResult> {
